...
for year in years:
    ...
    for month in months:
        ...
        for row in data.itertuples(index=False):
            ...
            if row.subreddit != 'politics' or pd.isna(comment):
                continue
            
            #consider only if comment has at least 5 words of length >= 3
            words_longer_than_3 = [word for word in comment.split() if len(word) >= 3]
            if len(words_longer_than_3) < 5:
                continue
...
samples.to_csv(args.dir+'/random_samples.csv', index=False, sep='\t')
